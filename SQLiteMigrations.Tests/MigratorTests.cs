...
namespace SQLiteMigrations.Tests
{
    ...
    public class MigratorTests
    {
		...
		public void MigrationsTableCreated()
		{
            ...
            {
				var exists = db.Query("SELECT COUNT(*) FROM sqlite_master WHERE name = ? AND type = ?", "__migrations", "table").SelectScalarInt().First() > 0;
		...
		public void MigrationIdSetted()
		{
			...
			using (var db = SQLite3.Open(DBPath))
			{
				var current = db.Query("SELECT id FROM __migrations ORDER BY id DESC LIMIT 1").SelectScalarInt().First();
